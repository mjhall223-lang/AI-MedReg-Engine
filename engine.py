    ...
    for root, dirs, files in os.walk(root_folder):
        for file in files:
            if file.endswith(".pdf"):
                # Inclusion Logic:
                # 1. Is it a general framework folder the user picked?
                # 2. OR is it a specific Federal PDF the user toggled on?
                is_framework = any(f.split()[0].lower() in root.lower() for f in selected_frameworks)
