...
def scout_organization(org_name, llm):
    ...
    try:
        with DDGS() as ddgs:
            ...
            query = f"March 2026 {org_name} AI layoffs headcount clinical trial SB 24-205"
            results = list(ddgs.text(query, max_results=5))
            news_text = "\n\n".join([f"{r['title']}: {r['body']}" for r in results])
    except: news_text = "Search offline."

    analysis_prompt = f"""
    Analyze '{org_name}' for its 2026 Statutory Debt. 
    Context: {news_text[:1500]}
    
    REQUIRED: Industry | Beast Count | Specialist Hole
...
def perform_gap_analysis(file_content, llm):
    """MECHANIC: Finds holes in uploaded technical/policy files."""
    gap_prompt = f"""
