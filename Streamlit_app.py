...
with tab2:
    ...
    if st.button("🔍 Scout & Auto-Calculate"):
        with st.status("Sifting 2026 news..."):
            ...
            # THE FIX: This updates the session state before the rerun
            st.session_state.headcount = found_count 
            
            total_debt = EconomicImpact.calculate_liability(found_count)['total']
            # Hard-hitting specialist prompt
            prompt = f"""
            Today is March 15, 2026. Lead: {co_name}. Headcount: {found_count}. Risk: ${total_debt:,}. News: {news_text}.
            Draft a liability-focused pitch for the June 30 Colorado AI Act deadline. 
...
# RENDER SIDEBAR AFTER LOGIC TO SHOW UPDATED NUMBERS
with st.sidebar:
